...
class BaseHostingService(ABC):
    ...
    def status_payload(self, project):
        if not project.link_is_active or project.server_status == HostedProject.ServerStatus.OFFLINE:
            ...
            state = "Degraded"
        elif project.status in {HostedProject.Status.DISABLED, HostedProject.Status.SUSPENDED, HostedProject.Status.MAINTENANCE}:
            state = "Offline"
        else:
            state = "Running"
        return {
    ...
    def _toggle_first_link(self, project_id, enabled, pending_state):
        from apps.hosting.providers import ProviderError, toggle_hosted_project_access

        project = self._project(project_id)
        try:
            toggle_hosted_project_access(
                project,
                enabled,
                user=self.user,
                reason=f"{self.provider or 'provider'} {pending_state.lower()} requested.",
                source=f"hosting.provider.{self.provider or 'unknown'}",
            )
        except ProviderError as exc:
            raise ProviderConnectionError(str(exc), code="provider_error", status_code=502, payload={"errors": exc.details}) from exc
        project.refresh_from_db()
        project.last_checked_at = timezone.now()
        project.save(update_fields=["last_checked_at"])
