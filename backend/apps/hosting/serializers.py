...
MAX_PROJECT_UPLOAD_BYTES = 5 * 1024 * 1024 * 1024
ALLOWED_SINGLE_UPLOAD_EXTENSIONS = {
    ".zip",
    ".gif",
    ".png",
    ".jpg",
...
class HostingHealthCheckSerializer(serializers.ModelSerializer):
    class Meta:
        model = HostingHealthCheck
        fields = "__all__"


class HostingIncidentSerializer(serializers.ModelSerializer):
    project_name = serializers.CharField(source="project.name", read_only=True)

    class Meta:
        model = HostingIncident
        fields = "__all__"
...
class ProjectUploadSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        uploaded_files = request.FILES.getlist("upload") if request else []
        relative_paths = request.data.getlist("paths") if request and hasattr(request.data, "getlist") else []
        source_type = str(request.data.get("source_type") or "").lower() if request else ""
        _validate_upload_batch(uploaded_files, relative_paths, source_type)
        file_obj = _folder_archive(uploaded_files, relative_paths) if len(uploaded_files) > 1 else validated_data["upload"]
        validated_data["upload"] = file_obj
...
def _folder_archive(files, relative_paths):
    buffer = tempfile.SpooledTemporaryFile(max_size=64 * 1024 * 1024)
    root_name = "project-upload"
    with zipfile.ZipFile(buffer, "w", zipfile.ZIP_DEFLATED) as archive:
        for index, file_obj in enumerate(files):
            raw_name = relative_paths[index] if index < len(relative_paths) and relative_paths[index] else file_obj.name
            safe_name = _safe_archive_name(raw_name)
            if "/" in safe_name:
                root_name = safe_name.split("/", 1)[0] or root_name
            with archive.open(safe_name, "w") as target:
                for chunk in file_obj.chunks():
                    target.write(chunk)
    buffer.seek(0)
    return File(buffer, name=f"{_safe_archive_name(root_name).split('/')[0] or 'project-upload'}.zip")


def _validate_upload_batch(files, relative_paths, source_type):
    if not files:
        raise ValidationError("At least one upload file is required.")
    max_bytes = int(getattr(settings, "MAX_PROJECT_UPLOAD_BYTES", MAX_PROJECT_UPLOAD_BYTES))
    total_size = sum(int(getattr(file_obj, "size", 0) or 0) for file_obj in files)
    if total_size > max_bytes:
        raise ValidationError(f"Project uploads are limited to {max_bytes // (1024 * 1024)}MB.")
    safe_paths = []
    for index, file_obj in enumerate(files):
        raw_name = relative_paths[index] if index < len(relative_paths) and relative_paths[index] else file_obj.name
        safe_name = _safe_archive_name(raw_name)
        if not safe_name:
            raise ValidationError("Upload contains an invalid file name.")
        if safe_name in safe_paths:
            raise ValidationError(f"Duplicate upload path rejected: {safe_name}.")
        safe_paths.append(safe_name)
    if len(files) == 1 and source_type not in {"folder", "directory"}:
        extension = "." + files[0].name.rsplit(".", 1)[-1].lower() if "." in files[0].name else ""
        if extension not in ALLOWED_SINGLE_UPLOAD_EXTENSIONS:
            raise ValidationError("Single-file uploads must be a ZIP, supported media asset, or supported static/code file.")


def _safe_archive_name(value):
    parts = []
    for part in str(value or "file").replace("\\", "/").split("/"):
        clean = "".join(ch if ch.isalnum() or ch in "._-" else "-" for ch in part).strip(".")
...
class HostedProjectSerializer(serializers.ModelSerializer):
    ...
    def get_recent_health_checks(self, obj):
        return HostingHealthCheckSerializer(obj.health_checks.all()[:5], many=True).data

    def get_open_incident(self, obj):
        incident = obj.incidents.filter(status=HostingIncident.Status.OPEN).first()
        return HostingIncidentSerializer(incident).data if incident else None
