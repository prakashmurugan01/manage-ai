...
class HostedProject(models.Model):
    class Platform(models.TextChoices):
        ...
        GITHUB = "github", "GitHub Deployments"
...
class HostingHealthCheck(models.Model):
    class TargetType(models.TextChoices):
        ...
        LINK = "link", "Hosting link"
        VERCEL_LINK = "vercel_link", "Vercel link"

    project = models.ForeignKey(HostedProject, related_name="health_checks", on_delete=models.CASCADE)
    hosting_link = models.ForeignKey("HostingLink", related_name="health_checks", on_delete=models.SET_NULL, null=True, blank=True)
    vercel_link = models.ForeignKey("VercelProjectLink", related_name="health_checks", on_delete=models.SET_NULL, null=True, blank=True)
    target_type = models.CharField(max_length=24, choices=TargetType.choices, default=TargetType.PROJECT, db_index=True)
    ...
    class Meta:
        ...
        indexes = [
            ...
            models.Index(fields=["target_type", "is_online", "-checked_at"]),
        ]


class HostingIncident(models.Model):
    class Status(models.TextChoices):
        OPEN = "open", "Open"
        RESOLVED = "resolved", "Resolved"

    project = models.ForeignKey(HostedProject, related_name="incidents", on_delete=models.CASCADE)
    hosting_link = models.ForeignKey("HostingLink", related_name="incidents", on_delete=models.SET_NULL, null=True, blank=True)
    vercel_link = models.ForeignKey("VercelProjectLink", related_name="incidents", on_delete=models.SET_NULL, null=True, blank=True)
    ...
    class Meta:
        ordering = ["-started_at"]
        indexes = [models.Index(fields=["project", "status", "-started_at"])]
...
class ProjectUploadSession(models.Model):
    class Status(models.TextChoices):
        PENDING = "pending", "Pending"
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    owner = models.ForeignKey(settings.AUTH_USER_MODEL, related_name="hosting_upload_sessions", on_delete=models.SET_NULL, null=True, blank=True)
    original_name = models.CharField(max_length=255)
    source_type = models.CharField(max_length=32, default="file", db_index=True)
    size_bytes = models.PositiveBigIntegerField(default=0)
    ...
    temp_dir = models.CharField(max_length=500, blank=True)
    status = models.CharField(max_length=24, choices=Status.choices, default=Status.PENDING, db_index=True)
    created_upload = models.OneToOneField(ProjectUpload, related_name="upload_session", on_delete=models.SET_NULL, null=True, blank=True)
    error_message = models.TextField(blank=True)
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    ...
    def completed_chunk_numbers(self):
        return {int(index) for index in self.received_chunks or []}

    @property
    def missing_chunks(self):
        completed = self.completed_chunk_numbers
        return [index for index in range(self.total_chunks or 0) if index not in completed]
...
class DeploymentRun(models.Model):
    ...
    primary_provider = models.CharField(max_length=32, blank=True, default="", db_index=True)
    backup_provider = models.CharField(max_length=32, blank=True)
    provider_deployment_id = models.CharField(max_length=180, blank=True, db_index=True)
    build_id = models.CharField(max_length=180, blank=True, db_index=True)
