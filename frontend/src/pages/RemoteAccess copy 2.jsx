...
export default function RemoteAccess() {
  ...
  useEffect(() => {
    ...
    socket.onmessage = (event) => {
      ...
      if (message.type === "file.result") setFileState({ ...normalizeFileResult(message.result), loading: false });
      if (message.type === "transfer.progress") handleTransferMessage(message, false);
      if (message.type === "clipboard.data") receiveRemoteClipboard(message.text || "");
      if (message.type === "device.storage.changed") {
        setFileState((prev) => (prev.path === "Drives" || !prev.path ? { path: "Drives", entries: message.drives || [], loading: false } : prev));
        setEvents((prev) => [{ type: "USB/storage changed", detail: message.message || "Disk Explorer refreshed." }, ...prev].slice(0, 12));
      }
  ...
  const renderBinaryFrame = async (blob) => {
    ...
    if (header.type === "transfer.chunk.binary") {
      mergeRemoteDownloadChunk({ ...header, chunkBytes: new Uint8Array(buffer.slice(4 + headerLength)) });
      return;
    }
  ...
  const mergeRemoteDownloadChunk = (message) => {
    ...
    if (!job) return;
    if (message.total) job.total = Number(message.total);
    if (message.name) job.name = message.name;
    if (message.packaging) {
      upsertTransferItem(job.key, { name: job.name, status: "packaging", percent: 0, uploadedBytes: 0 });
      return;
    }
    const bytes = Number(message.bytes || job.received || 0);
    if (message.chunkBytes && bytes > job.received) {
      job.chunks.push(message.chunkBytes);
      job.received = bytes;
    } else if (message.chunk && bytes > job.received) {
    ...
    if (message.complete) {
      if (job.total && job.received !== job.total) {
        upsertTransferItem(job.key, { status: "failed", error: `Transfer ended at ${formatBytes(job.received)} of ${formatBytes(job.total)}.` });
        setEvents((prev) => [{ type: "Download integrity failed", detail: job.name }, ...prev].slice(0, 12));
        return;
      }
  ...
  const startRemoteFileDownload = async (entry) => {
    ...
    try {
      ...
      setEvents((prev) => [{ type: isFolder ? "Folder download started" : "Download started", detail: downloadName }, ...prev].slice(0, 12));
    } catch (error) {
  ...
  const startBulkDownload = (entries = []) => {
    entries.filter(Boolean).forEach((entry) => startRemoteFileDownload(entry));
  };
  ...
  const receiveRemoteClipboard = async (text) => {
    try {
      await navigator.clipboard?.writeText(text || "");
      setEvents((prev) => [{ type: "Clipboard synced", detail: "Remote clipboard copied locally." }, ...prev].slice(0, 12));
    } catch {
      setEvents((prev) => [{ type: "Clipboard ready", detail: "Browser permission blocked automatic clipboard write." }, ...prev].slice(0, 12));
    }
  };

  const pointerButton = (event) => (event.button === 2 ? "right" : event.button === 1 ? "middle" : "left");

  const pointerRatios = (event) => {
    const box = screenRef.current.getBoundingClientRect();
    return {
      x_ratio: Math.max(0, Math.min(1, (event.clientX - box.left) / box.width)),
      y_ratio: Math.max(0, Math.min(1, (event.clientY - box.top) / box.height))
    };
  };
  ...
  const handleScreenClick = (event) => {
    ...
    sendControl("mouse", { action: "click", button: pointerButton(event), ...pointerRatios(event) });
  };

  const handleScreenContextMenu = (event) => {
    if (!screenRef.current || !canControl) return;
    event.preventDefault();
    screenRef.current.focus();
    sendControl("mouse", { action: "click", button: "right", ...pointerRatios(event) });
  };
  ...
  const handleScreenMouseMove = (event) => {
    ...
    stats.lastMove = now;
    sendControl("mouse", { action: "move", ...pointerRatios(event) });
  };
  ...
  const handleScreenMouseDown = (event) => {
    ...
    sendControl("mouse", { action: "down", button: pointerButton(event), ...pointerRatios(event) });
  };

  const handleScreenMouseUp = (event) => {
    if (!screenRef.current || !canControl) return;
    frameStatsRef.current.mouseDown = false;
    sendControl("mouse", { action: "up", button: pointerButton(event), ...pointerRatios(event) });
  };
  ...
  const handleScreenKeyDown = async (event) => {
    if (!canControl) return;
    event.preventDefault();
    ...
    if ((event.ctrlKey || event.metaKey) && ["c", "x"].includes(event.key.toLowerCase())) {
      sendControl("keyboard", { key: event.key.toLowerCase(), modifiers: ["ctrl"], event: "press" });
      window.setTimeout(() => sendControl("clipboard.get", {}), 250);
      return;
    }
    if (["Control", "Shift", "Alt", "Meta"].includes(event.key)) {
      sendControl("keyboard", { key: specialKeys[event.key], event: "down" });
      return;
    }
  ...
  return (
    <div className="space-y-5 text-[color:var(--text)]">
      <header className="relative overflow-hidden rounded-[1.75rem] border border-cyan-300/15 bg-slate-950/60 p-5 shadow-2xl shadow-cyan-950/30 backdrop-blur-2xl sm:p-6">
        <div className="pointer-events-none absolute inset-0 cyber-grid opacity-35" />
        <div className="pointer-events-none absolute -right-16 -top-20 h-64 w-64 rounded-full bg-fuchsia-500/20 blur-3xl" />
        <div className="relative flex flex-wrap items-start justify-between gap-4">
          <div>
            <div className="flex flex-wrap items-center gap-2">
              <span className="inline-flex items-center gap-2 rounded-full border border-cyan-300/20 bg-cyan-300/10 px-3 py-1 text-xs font-black uppercase tracking-[0.2em] text-cyan-100">
                <ShieldCheck size={14} /> Secure desktop agent
              </span>
              <span className="inline-flex items-center gap-2 rounded-full border border-emerald-300/20 bg-emerald-300/10 px-3 py-1 text-xs font-bold text-emerald-100">
                <Lock size={13} /> Approval gated
              </span>
            </div>
            <h1 className="mt-4 text-3xl font-black tracking-tight text-white sm:text-5xl">Remote Access Command Center</h1>
            <p className="mt-3 max-w-3xl text-sm leading-6 text-slate-400">
              Low-latency remote desktop, full approved input control, all-drive file exploration, chunked transfers, and live device telemetry.
            </p>
          </div>
          <div className="flex flex-wrap gap-2">
            <button type="button" onClick={enableTurboMode} className={`inline-flex items-center gap-2 rounded-xl border px-4 py-2 text-sm font-black transition ${turboMode ? "border-fuchsia-300/50 bg-fuchsia-300/20 text-fuchsia-50 shadow-lg shadow-fuchsia-500/20" : "border-white/10 bg-white/[0.04] text-slate-200 hover:border-cyan-300/40"}`}>
              <Activity size={16} /> Turbo Mode
            </button>
      ...
      <section className="grid gap-3 md:grid-cols-4">
        <TelemetryCard icon={Cpu} label="CPU" value={telemetry.cpu} suffix="%" tone="cyan" />
        <TelemetryCard icon={Activity} label="RAM" value={telemetry.ram} suffix="%" tone="violet" />
        <TelemetryCard icon={HardDrive} label="Disk" value={telemetry.disk} suffix="%" tone="amber" />
        <TelemetryCard icon={Battery} label="Battery" value={telemetry.battery} suffix="%" tone="green" />
      </section>
              ...
              <div className="space-y-3">
                ...
                <div className="rounded-lg border border-white/10 bg-white/[0.03] p-3">
                  ...
                  <div className="grid gap-3 text-xs text-slate-400">
                    <label>FPS <input type="range" min="4" max="30" value={streamSettings.fps} onChange={(event) => setStreamSettings((prev) => ({ ...prev, fps: Number(event.target.value) }))} className="w-full" /> <span className="text-cyan-100">{streamSettings.fps}</span></label>
                    <label>Quality <input type="range" min="45" max="92" value={streamSettings.quality} onChange={(event) => setStreamSettings((prev) => ({ ...prev, quality: Number(event.target.value) }))} className="w-full" /> <span className="text-cyan-100">{streamSettings.quality}</span></label>
                    <label>Width
            ...
            <Panel title="Disk Explorer" action={canFiles && <button onClick={() => requestFiles("drives")} className="rounded-md border border-white/10 bg-white/[0.04] p-2 text-slate-200 hover:border-cyan-300/50"><HardDrive size={16} /></button>}>
              {!canFiles ? (
                ...
                <FileExplorer
                  ...
                  onBulkDownload={startBulkDownload}
...
function FileExplorer({ state, transfers, onOpen, onUpload, onDelete, onDownload, onEntryDownload, onBulkDownload, onPauseDownload }) {
  ...
  const isDriveRoot = state.path === "Drives";
  const driveEntries = (state.entries || []).filter((entry) => entry.drive);
  const normalEntries = (state.entries || []).filter((entry) => !entry.drive);
  const filteredEntries = normalEntries.filter((entry) => `${entry.name || ""} ${entry.type || ""}`.toLowerCase().includes(search.toLowerCase()));
  const downloadableEntries = isDriveRoot ? driveEntries : normalEntries;
  const selectedEntries = downloadableEntries.filter((entry) => selectedPaths.includes(entry.path));
  const breadcrumbs = buildBreadcrumbs(state.path);
  const currentFolderEntry = !isDriveRoot && state.path ? { name: breadcrumbs.at(-1)?.label || "folder", path: state.path, is_dir: true } : null;
  useEffect(() => {
    setSelectedPaths([]);
    setSearch("");
  }, [state.path]);
  ...
  const toggleAll = () => {
    setSelectedPaths((prev) => (prev.length === downloadableEntries.length ? [] : downloadableEntries.map((entry) => entry.path)));
  };
    ...
    <div>
      <div className="mb-3 flex flex-wrap items-center justify-between gap-2 text-xs text-slate-400">
        <div className="flex items-center gap-2">
          ...
          <div className="flex flex-wrap items-center gap-1">
            {breadcrumbs.length ? breadcrumbs.map((crumb, index) => (
              <button key={`${crumb.path}-${index}`} type="button" onClick={() => crumb.path && onOpen?.(crumb.path)} className="rounded px-1 text-slate-300 hover:bg-white/[0.06] hover:text-cyan-100">
                {index ? "/" : ""} {crumb.label}
              </button>
        ...
        <div className="flex gap-2">
          {currentFolderEntry && (
            <button type="button" onClick={() => onEntryDownload?.(currentFolderEntry)} className="inline-flex items-center gap-1 rounded-md border border-cyan-300/25 bg-cyan-300/10 px-2 py-1 text-cyan-100 hover:border-cyan-300/50">
              <FileArchive size={13} /> Download All
            </button>
          )}
          {downloadableEntries.length ? (
            <button type="button" onClick={toggleAll} className="inline-flex items-center gap-1 rounded-md border border-white/10 bg-white/[0.04] px-2 py-1 text-slate-200 hover:border-cyan-300/40">
              <CheckCircle2 size={13} /> {selectedPaths.length === downloadableEntries.length ? "Clear" : "Select All"}
            </button>
          ) : null}
          {selectedEntries.length ? (
            <button type="button" onClick={() => onBulkDownload?.(selectedEntries)} className="inline-flex items-center gap-1 rounded-md border border-cyan-300/25 bg-cyan-300/10 px-2 py-1 text-cyan-100 hover:border-cyan-300/50">
              <Download size={13} /> Download {selectedEntries.length}
            </button>
          ) : null}
      ...
      {isDriveRoot && driveEntries.length ? (
        <DriveSections drives={driveEntries} selectedPaths={selectedPaths} onSelect={toggleSelected} onOpen={onOpen} onDownload={onEntryDownload} />
      ) : null}

      {!isDriveRoot && (
        <div className="mb-3 flex flex-wrap items-center justify-between gap-2 rounded-lg border border-white/10 bg-white/[0.025] p-2">
          <input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search this folder" className="min-w-0 flex-1 rounded-md border border-white/10 bg-slate-950 px-3 py-2 text-sm text-slate-100 outline-none focus:border-cyan-300/50" />
          <div className="flex rounded-md border border-white/10 bg-slate-950 p-1 text-xs">
            <button type="button" onClick={() => setViewMode("list")} className={`rounded px-2 py-1 ${viewMode === "list" ? "bg-cyan-300/15 text-cyan-100" : "text-slate-400"}`}>List</button>
            <button type="button" onClick={() => setViewMode("grid")} className={`rounded px-2 py-1 ${viewMode === "grid" ? "bg-cyan-300/15 text-cyan-100" : "text-slate-400"}`}>Grid</button>
          </div>
        </div>
      )}

      <div className="max-h-[520px] overflow-auto rounded-lg border border-white/10">
        {viewMode === "grid" && !isDriveRoot ? (
          <div className="grid gap-2 p-2 sm:grid-cols-2 lg:grid-cols-3">
            {filteredEntries.map((entry) => (
              <div key={entry.path} onDoubleClick={() => entry.is_dir && onOpen(entry.path)} className="rounded-lg border border-white/10 bg-slate-950/60 p-3">
                <div className="flex items-start justify-between gap-2">
                  <button type="button" onClick={() => entry.is_dir && onOpen(entry.path)} className="flex min-w-0 items-center gap-2 text-left text-slate-200">
                    {entry.is_dir ? <Folder size={18} className="text-cyan-300" /> : <File size={18} className="text-slate-400" />}
                    <span className="truncate text-sm font-medium">{entry.name}</span>
                  </button>
                  <input type="checkbox" checked={selectedPaths.includes(entry.path)} onChange={() => toggleSelected(entry.path)} className="h-4 w-4 rounded border-white/20 bg-slate-950 accent-cyan-300" />
                </div>
                <p className="mt-2 text-xs text-slate-500">{entry.is_dir ? "Folder" : formatBytes(entry.size)}{entry.type ? ` · ${entry.type}` : ""}</p>
                <div className="mt-3 flex gap-2">
                  <button type="button" onClick={() => onEntryDownload?.(entry)} className="inline-flex items-center gap-1 rounded-md border border-cyan-300/25 bg-cyan-300/10 px-2 py-1 text-xs text-cyan-100"><Download size={13} /> Download</button>
                  <button type="button" disabled={entry.is_dir} onClick={() => onDelete(entry.path)} className="text-xs text-red-300 disabled:text-slate-600">Delete</button>
                </div>
              </div>
            ))}
          </div>
        ) : filteredEntries.map((entry) => (
          <div key={entry.path} onContextMenu={(event) => { event.preventDefault(); onEntryDownload?.(entry); }} className="grid grid-cols-[auto_minmax(0,1fr)_auto_auto_auto] items-center gap-3 border-b border-white/5 px-3 py-2 text-sm last:border-b-0">
            <input type="checkbox" checked={selectedPaths.includes(entry.path)} onChange={() => toggleSelected(entry.path)} className="h-4 w-4 rounded border-white/20 bg-slate-950 accent-cyan-300" />
            ...
            <span className="text-xs text-slate-500">{entry.is_dir ? "Folder" : formatBytes(entry.size)}{entry.type ? ` · ${entry.type}` : ""}</span>
            <button
              onClick={() => onEntryDownload?.(entry)}
              title={entry.is_dir ? "Download folder as ZIP" : "Download file"}
              className="inline-flex items-center gap-1 rounded-md border border-white/10 bg-white/[0.04] px-2 py-1 text-xs text-cyan-100 hover:border-cyan-300/40"
            >
        ...
        {!filteredEntries.length && !driveEntries.length && <div className="p-6"><Empty title={state.loading ? "Loading folder" : "No directory loaded"} detail={state.loading ? "Fetching files from the remote device." : "Use the drive button to ask the agent for available disks."} /></div>}
        {!normalEntries.length && driveEntries.length ? <div className="p-4 text-center text-xs text-slate-500">Select a drive card to open its root folder.</div> : null}
      </div>
    </div>
  );
}

function DriveSections({ drives, selectedPaths, onSelect, onOpen, onDownload }) {
  const groups = [
    ["Internal Drives", drives.filter((drive) => ["fixed"].includes(drive.drive_type) || ["SSD", "HDD", "Internal Drive"].includes(drive.category))],
    ["Android Devices", drives.filter((drive) => drive.drive_type === "portable" || /android|portable|mtp|ptp/i.test(`${drive.category} ${drive.connection_type}`))],
    ["USB Devices", drives.filter((drive) => drive.drive_type !== "portable" && (drive.drive_type === "removable" || /usb|sd card/i.test(`${drive.category} ${drive.connection_type}`)))],
    ["Network Storage", drives.filter((drive) => drive.drive_type === "network")]
  ].filter(([, items]) => items.length);
  return (
    <div className="mb-3 space-y-4">
      {groups.map(([title, items]) => (
        <div key={title}>
          <p className="mb-2 text-xs font-bold uppercase tracking-[0.16em] text-slate-500">{title}</p>
          <div className="grid gap-3 sm:grid-cols-2">
            {items.map((drive) => <DriveCard key={drive.path} drive={drive} selected={selectedPaths.includes(drive.path)} onSelect={() => onSelect(drive.path)} onOpen={onOpen} onDownload={onDownload} />)}
          </div>
        </div>
      ))}
    </div>
...
function DriveCard({ drive, selected, onSelect, onOpen, onDownload }) {
  const percent = Math.max(0, Math.min(100, Number(drive.usage_percent || 0)));
  const tone = drive.health === "critical" ? "rose" : drive.health === "warning" ? "amber" : drive.health === "locked" ? "slate" : "emerald";
  const colors = {
  ...
  return (
    <div onClick={() => onOpen(drive.path)} className={`group relative cursor-pointer overflow-hidden rounded-2xl border bg-slate-950/70 p-4 text-left shadow-lg transition hover:-translate-y-0.5 hover:border-cyan-300/45 hover:shadow-cyan-500/10 ${selected ? "border-cyan-300/60" : "border-cyan-300/15"}`}>
      <div className="absolute -right-8 -top-8 h-24 w-24 rounded-full bg-cyan-400/15 blur-2xl transition group-hover:bg-fuchsia-400/20" />
      <div className="relative flex items-start justify-between gap-3">
        <button type="button" onClick={(event) => { event.stopPropagation(); onOpen(drive.path); }} className="grid h-11 w-11 place-items-center rounded-2xl border border-cyan-300/20 bg-cyan-300/10 text-cyan-100">
          <HardDrive size={20} />
        </button>
        <span className={`rounded-full border px-2 py-1 text-[10px] font-black uppercase tracking-[0.14em] ${colors[tone]}`}>{drive.health || "unknown"}</span>
      </div>
      <button type="button" onClick={(event) => { event.stopPropagation(); onOpen(drive.path); }} className="relative mt-4 block max-w-full truncate text-left text-lg font-black text-white">{drive.name}</button>
      <p className="relative mt-1 text-xs uppercase tracking-[0.16em] text-slate-500">{drive.drive_type || "drive"}</p>
      <div className="relative mt-4">
        <div className="mb-1 flex justify-between text-[11px] text-slate-400">
          <span>{formatBytes(drive.free || 0)} free</span>
          <span>{formatBytes(drive.total || 0)}</span>
        </div>
        <div className="h-2 overflow-hidden rounded-full bg-slate-800">
          <div className={`h-full ${tone === "rose" ? "bg-rose-300" : tone === "amber" ? "bg-amber-300" : "bg-cyan-300"}`} style={{ width: `${percent}%` }} />
        </div>
        <p className="mt-2 text-xs text-slate-500">{percent}% used</p>
      </div>
      <div className="relative mt-3 flex gap-2">
        <button type="button" onClick={(event) => { event.stopPropagation(); onSelect?.(); }} className="rounded-md border border-white/10 bg-white/[0.04] px-2 py-1 text-xs text-slate-200 hover:border-cyan-300/40">{selected ? "Selected" : "Select"}</button>
        <button type="button" onClick={(event) => { event.stopPropagation(); onDownload?.(drive); }} className="inline-flex items-center gap-1 rounded-md border border-cyan-300/25 bg-cyan-300/10 px-2 py-1 text-xs text-cyan-100 hover:border-cyan-300/50"><Download size={13} /> Drive ZIP</button>
      </div>
    </div>
  );
}
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="mt-2 flex flex-wrap items-center gap-3 text-[11px] text-slate-500">
            ...
            {item.status === "packaging" && <span>Compressing folder</span>}
...
function TelemetryCard({ icon: Icon, label, value, suffix = "", tone = "cyan" }) {
  const numeric = value === null || value === undefined ? null : Number(value);
  const safe = numeric === null || Number.isNaN(numeric) ? 0 : Math.max(0, Math.min(100, numeric));
  const colors = {
  ...
  return (
    <div className="rounded-2xl border border-white/10 bg-slate-950/55 p-4 shadow-xl backdrop-blur">
      <div className="flex items-center justify-between gap-3">
        <span className={`grid h-10 w-10 place-items-center rounded-xl border ${colors[tone]}`}><Icon size={18} /></span>
        <span className="text-xl font-black text-white">{numeric === null || Number.isNaN(numeric) ? "n/a" : `${Math.round(numeric)}${suffix}`}</span>
      </div>
...
function safeDownloadName(value = "download") {
  return String(value).replace(/[<>:"/\\|?*]+/g, "_").replace(/\s+/g, " ").trim() || "download";
}

function buildBreadcrumbs(path = "") {
  if (!path || path === "Drives") return path === "Drives" ? [{ label: "Drives", path: "" }] : [];
  if (path.startsWith("shell:")) {
    const parts = path.removeprefix ? path.removeprefix("shell:").split("/") : path.replace(/^shell:/, "").split("/");
    return parts.filter(Boolean).map((part, index) => ({
      label: index === 0 ? "Portable Device" : part.startsWith("@") ? `Folder ${Number(part.slice(1)) + 1 || index}` : part,
      path: `shell:${parts.slice(0, index + 1).join("/")}`
    }));
  }
